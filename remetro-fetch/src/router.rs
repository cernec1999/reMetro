...
impl IntoResponse for StationDirectoryError {
    fn into_response(self) -> axum::response::Response {
        let status_code = match self {
            StationDirectoryError::InvalidStationOrPlatform(_) => axum::http::StatusCode::NOT_FOUND,
...
async fn station_track_predictions_handler(
    ...
    // convert platform_code to an int
    let track_code = track_code.parse::<u8>().unwrap_or(u8::MAX);
    let request =
        TrainPredictionsRequest::StationPlatform(station_code, WMATATrackCode(track_code));
    let predictions = station_directory.predictions(&request)?;
    Ok(Json(predictions))
}
...
async fn station_tracks(
    ...
    let tracks = station_directory.station_platforms(station_code)?;
    Ok(Json(tracks))
}
